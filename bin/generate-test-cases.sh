...
./bin/kafka_broker_generator.py \
  ...
  --num-brokers 1 \
  --num-triggers 1 \
  --resources-output-dir tests/broker/kafka/p10-r3-unord-b1-t1-64kb-namespaced/resources \
  --hf-output-dir tests/broker/kafka/p10-r3-unord-b1-t1-64kb-namespaced \
  --name-prefix p10-r3-unord-b1-t1-64kb-namespaced \
  --payload-file payloads/payload.68KB.txt \
  --delivery-order unordered
